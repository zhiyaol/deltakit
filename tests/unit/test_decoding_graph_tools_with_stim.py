...
from deltakit_core.decoding_graphs._decoding_graph_tools import (
    compute_graph_distance, filter_to_data_edges, filter_to_measure_edges,
    graph_to_json, nx_graph_from_json, unweight_graph)
from deltakit_decode.utils._graph_circuit_helpers import (
    parse_stim_circuit, stim_circuit_to_graph_dem)


class TestGraphToJSON():

    def test_2_boundary_graph_to_json_raises_exception(self):
        graph = NXDecodingGraph.from_edge_list([(i, (i + 1) % 17) for i in range(17)],
                                               boundaries=[16, 8])
        logicals = [[(4, 5)]]
        with pytest.raises(ValueError,
                           match="JSON graph representation supports maximum one boundary"):
    ...
    def stim_circuit(self, request):
        distance = 5
        stim_circ = stim.Circuit.generated(request.param,
    ...
    def test_full_graph_to_json_on_code_tasks_matches_original_graph(self, stim_circuit):
        ...
        assert all(log == rec_log for log, rec_log in zip(
            logicals, reconstructed_logicals))
...
def test_compute_graph_distance(code_task, distance):
    stim_circ = stim.Circuit.generated(code_task,
                                         distance=distance, rounds=distance,
                                         after_clifford_depolarization=0.01,
                                         before_measure_flip_probability=0.01,
                                         before_round_data_depolarization=0.01)
...
@pytest.mark.parametrize("circuit_path, expected_distance", [
...
def test_compute_graph_distance_from_file(circuit_path, expected_distance, reference_data_dir):
...
def test_unweight_graph(code_task):
    ...
    assert all(record["weight"] == 1
               for record in unweighted_graph.edge_records.values())
