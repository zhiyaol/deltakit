...
from deltakit.circuit import PauliX


def draw_patch(self, data_qubit_click_callback = lambda _: None, ancilla_qubit_click_callback = lambda _: None, filename: Optional[str] = None, unrotated_code: bool = False) -> go.FigureWidget:
    """
    ...
    pauli_marker_colors = ['red', 'lime', 'blue']
    ...
    def update_data_point(trace, points, selector):
        ...
        for i in points.point_inds:
            if c[i] == pauli_marker_colors[-1]: # If on last Pauli color, unhighlight point
                c[i] = DrawingColours.DATA_QUBIT_COLOUR.value
                s[i] = 20
            else:
                if c[i] in pauli_marker_colors: # If a Pauli color, change to next
                    c[i] = pauli_marker_colors[pauli_marker_colors.index(c[i]) + 1]
                else: # If not a Pauli color, change to first Pauli color
                    c[i] = pauli_marker_colors[0]
                s[i] = 30
        with f.batch_update():
            data_scatter.marker.color = c
            data_scatter.marker.size = s
        data_qubit_click_callback(points)
    ...
    if x_ancilla_trace_idx is not None:
        ...
        def update_x_ancilla_point(trace, points, selector):
            ...
            ancilla_qubit_click_callback(points)
...
def update_patch(self, f: go.FigureWidget, data_qubit_click_callback = lambda _: None, ancilla_qubit_click_callback = lambda _: None, filename: Optional[str] = None, unrotated_code: bool = False) -> go.FigureWidget:
    """
    Updates a Plotly FigureWidget to display an interactive picture of the planar code using.
    Meant for updating the content of a `go.FigureWidget` previously generated by `draw_patch`.

    Parameters
    ----------
    f: go.FigureWidget
        The widget to update with the new interactive planar code. Its layout and data will be
        overwritten.
    filename: str, optional
        Path to the file where to save the pictorial representation of the
        planar code stored in this class. Will save as HTML format.
    ...
    # Update figure with new traces and layout
    with f.batch_update():
        f.data = ()
        f.add_traces(traces)
